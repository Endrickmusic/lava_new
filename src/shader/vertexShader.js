...
void main() {
    ...
    for(int i = 0; i < 5; i++) {
      ...
      float noiseFlow = 5. * float(i) * 0.2;
      float noiseSpeed = 10. * float(i) * 0.2;
      float noiseSeed = 1. * float(i) * 10.;
      vec2 noiseFreq = vec2(0.5, 0.6);
      float noiseFloor = 0.1;
      float noiseCeil = 1. + float(i) * 0.07;

      float noise = smoothstep(noiseFloor, noiseCeil,
      snoise(
        vec3(
          noiseCoord.x * noiseFreq.x + uTime * noiseFlow, 
          noiseCoord.y * noiseFreq.y + uTime * 0.35, 
